...
use cgmath::{MetricSpace, Quaternion, Vector1, Vector2, Vector3, Vector4};
...
impl InterpolateDerivative<f64> for Vector2<f64> {
  fn lerp(_t: f64, a: Self, b: Self) -> Self {
    b - a
  }
  ...
  fn cubic_hermite(
    ...
    let b1_der_t =
      1f64 / (t2 - t0) * (a2 - a1) + (t2 - t) / (t2 - t0) * a1_der + (t - t0) / (t2 - t0) * a2_der;
    let b2_der_t =
      1f64 / (t3 - t1) * (a3 - a2) + (t3 - t) / (t3 - t1) * a2_der + (t - t1) / (t3 - t1) * a3_der;
  ...
  fn cubic_bezier_second_derivative(t: f64, a: Self, u: Self, v: Self, b: Self) -> Self {
    6. * t * (b + 3. * (u - v) - a) + 6. * (a - 2. * u + v)
  }

  fn cubic_bezier_mirrored_second_derivative(t: f64, a: Self, u: Self, v: Self, b: Self) -> Self {
    Self::cubic_bezier_second_derivative(t, a, u, b + b - v, b)
  }
